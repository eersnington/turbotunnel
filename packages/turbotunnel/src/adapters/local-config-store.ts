...
function makeLocalConfigStore(path: string): Effect.Effect<LocalConfigStoreShape, never, FileSystem> {
...
function readConfig(
  fs: FileSystem,
  path: string,
): Effect.Effect<LocalConfig, ConfigFileReadError | ConfigFileParseError> {
  return Effect.gen(function* () {
