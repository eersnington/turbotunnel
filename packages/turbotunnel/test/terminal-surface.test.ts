...
describe("TerminalSurface", () => {
  it.effect("deduplicates repeated progress in plain mode", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(output).toContain("Tunnel ready");
  ...
  it.effect("stops cursor redraw after releasing the terminal to a child", () => {
    ...
    return Effect.gen(function* () {
      ...
      const outputAfterRelease = writes.slice(releaseIndex).join("");
      expect(outputAfterRelease).toContain("Waiting for localhost:5173");
