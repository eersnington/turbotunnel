...
| `tt dev` startup | 60s | Turbotunnel stops the managed process and tunnel after the deadline |
...
Vercel may run several gateway instances. Do not multiply the public WebSocket limit by an assumed instance count because the active instance count can change.

An HTTP request may still reach the local app before a `504` response. Avoid retrying non-idempotent requests unless your application can detect duplicates.
