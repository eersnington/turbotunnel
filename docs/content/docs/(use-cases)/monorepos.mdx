...
- CLI flags override project settings for a single run.
- Managed `vercel.app` hostnames attach to the gateway project during connect. Confirm the printed public URL if a requested hostname is unavailable.
- Inspect local tunnels with `tt status`. Discover recent sessions across machines with `tt list`.
