...
`tt deploy` provisions a gateway in your Vercel account and saves verified connection settings. It preserves the previous local configuration if deployment or verification fails. See [Configure your gateway](/docs/gateway-configuration) for the saved settings.
