...
`tt status` asks each local Turbotunnel process for its lifecycle state. It reports running tunnels, relay health, and gateway reachability.
...
The `version` field identifies the snapshot schema. Consumers should read only the counters they need.
