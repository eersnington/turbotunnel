...
`tt dev` opens a tunnel to a configured local port. With a command after `--`, it also runs that command as a managed child process. Turbotunnel does not discover package scripts, allocate ports, recognize frameworks, or change child arguments.
